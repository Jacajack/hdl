use std::io::Write;
use crate::{core::{CompilerError,IdTable,CommentTable, CommentTableKey,IdTableKey,numeric_constant_table::{NumericConstantTable,NumericConstantTableKey}}, ProvidesCompilerDiagnostic, lexer::NumericConstant};
...
impl<'extras> PrettyPrinterContext<'extras>{
    pub fn new(id_table: &'extras IdTable, comment_table: &'extras CommentTable, constant_table: &'extras NumericConstantTable, output: Box<dyn Write>) -> Self{
        Self{
            indentation_level: 0,
            id_table,
            comment_table,
    ...
    pub fn write_indent(&mut self, s: &str) -> miette::Result<()>{
        ...
        true => write!(self.output, "{}{}", " ".repeat(self.indentation_level), s).map_err(|e| CompilerError::IoError(e).to_diagnostic())?,
        false => self.write(s)?,
        }
