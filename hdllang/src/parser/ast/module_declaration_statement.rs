...
use super::{DirectDeclarator, TypeDeclarator, TypeQualifier, TypeSpecifier, DeclarationScope};
...
impl VariableDeclarationStatement{
	pub fn create_variable_declaration(&self, already_created: AlreadyCreated, nc_table: &NumericConstantTable) -> miette::Result<Vec<Variable>>{
		let kind = VariableKind::from_type_declarator(&self.type_declarator, already_created, nc_table)?;
...
impl VariableKind{
	pub fn from_type_declarator(
		...
		nc_table: &NumericConstantTable) -> miette::Result<Self>{
		use TypeSpecifier::*;
		match &type_declarator.specifier{
    		...
    		Int { location } => {
				Ok(VariableKind::Generic(GenericVariable { value: None, kind: GenericVariableKind::Int(*location) }))
			},
    		Wire { location } => {
				...
				use TypeQualifier::*;
				for qualifier in &type_declarator.qualifiers{
					match qualifier{
				...
				if already_created.direction == Direction::None{
					return Err(miette::Report::new(SemanticError::MissingDirectionQualifier.to_diagnostic_builder().label(type_declarator.location, "Wire must be either input or output").build()));
				}
				if already_created.sensitivity == SignalSensitivity::NoSensitivity{
					return Err(miette::Report::new(SemanticError::MissingSensitivityQualifier.to_diagnostic_builder().label(type_declarator.location, "Signal must be either const, clock, comb, sync or async").build()));
				}
...
impl ModuleDeclarationVariableBlock {
	pub fn create_variable_declaration(&self, mut already_created: AlreadyCreated, nc_table: &NumericConstantTable, declaration_scope: &DeclarationScope) -> miette::Result<Vec<Variable>>{
		use TypeQualifier::*;
		...
		for statement in &self.statements{
			variables.append(&mut statement.create_variable_declaration(already_created.clone(), nc_table, declaration_scope)?);
		}
		Ok(variables)
	}
}
...
impl ModuleDeclarationStatement {
	pub fn create_variable_declaration(&self, already_created: AlreadyCreated, nc_table: &NumericConstantTable, scope: &DeclarationScope) -> miette::Result<Vec<Variable>>{
		...
		match self{
			...
			VariableBlock(block) => block.create_variable_declaration(already_created, nc_table, scope),
		}
	}
	pub fn analyze(
		&self,
