...
impl InstantiationStatement {
	pub fn first_pass(
						...
						Generic(gen) => {
							gen.value = match new_sig {
								...
								None => {
									let id = local_ctx
										.scope
										.add_expression(scope_id, id_expr.expression.clone());
		...
		for (key, entry) in scope.get_all_expressions() {
			...
			let id = local_ctx
				.scope
				.add_expression(scope_id, entry_copy.expression);
		...
		for stmt in &self.port_bind {
			...
			interface_variable.var.kind.remap_bus_widths(&scope, &ctx.id_table, ctx.nc_table, &ids_map)?;
			debug!("Interface variable is {:?}", interface_variable.var.kind);
