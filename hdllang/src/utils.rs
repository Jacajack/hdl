...
pub fn combine(root_file_name: String, mut output: Box<dyn Write>) -> miette::Result<()> {
	...
	while let Some(file_name) = file_queue.pop_front() {
		...
		let (paths, ..) = crate::analyzer::combine(global_ctx, &root, String::from("."), &mut map)
...
pub fn compile(mut code: String, file_name: String, mut output: Box<dyn Write>) -> miette::Result<()> {
	...
	let (_, global_ctx, modules) = crate::analyzer::combine(global_ctx, &root, String::from("."), &mut map)
...
pub fn elaborate(
	...
	let (_, global_ctx, modules) =
		crate::analyzer::combine(global_ctx, &root, String::from("."), &mut map).map_err(|e| {
			if json_report {
				println!("{:?}", e);
			}
