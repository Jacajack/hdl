...
impl DiagnosticBuffer {
	...
	pub fn print_diagnostics(self, file_name: String, source_code: String) -> miette::Result<()>{
		...
		for diag in self.buffer {
			eprintln!(
				"{:?}",
				miette::Report::new(diag).with_source_code(miette::NamedSource::new(file_name.clone(), source_code.clone()))
			);
		}
		...
		return Err(miette::Report::new(self.error_buffer.last().unwrap().clone()).with_source_code(miette::NamedSource::new(file_name, source_code)).into());
	}
	pub fn new() -> Self {
		Self { buffer: vec![], error_buffer: vec![] }
	}
}
