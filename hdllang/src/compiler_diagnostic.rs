...
impl From<CompilerDiagnostic> for CompilerDiagnosticBuilder {
	fn from(diag: CompilerDiagnostic) -> Self { Self { diag: Some(diag) } }
}
...
impl CompilerDiagnosticBuilder {
	...
	pub fn new_error(msg: &str) -> Self { CompilerDiagnostic::new_error(msg).into() }

	/// Creates a new warning message
	pub fn new_warning(msg: &str) -> Self { CompilerDiagnostic::new_warning(msg).into() }

	/// Creates a new info message
	pub fn new_info(msg: &str) -> Self { CompilerDiagnostic::new_warning(msg).into() }
	...
	pub fn build(&mut self) -> CompilerDiagnostic { self.diag.take().unwrap() }
}

impl Display for CompilerDiagnostic {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result { write!(f, "{}", self.error_text) }
}
...
impl Diagnostic for CompilerDiagnostic {
	...
	fn url<'a>(&'a self) -> Option<Box<dyn Display + 'a>> { None }
	...
	fn related<'a>(&'a self) -> Option<Box<dyn Iterator<Item = &'a dyn Diagnostic> + 'a>> { None }

	fn diagnostic_source(&self) -> Option<&dyn Diagnostic> { None }
}
...
impl CompilerDiagnostic {
	...
	pub fn new_error(msg: &str) -> Self { Self::new(miette::Severity::Error, msg) }

	/// Creates a new warning diagnostic
	pub fn new_warning(msg: &str) -> Self { Self::new(miette::Severity::Warning, msg) }

	/// Creates a new info diagnostic
	pub fn new_info(msg: &str) -> Self { Self::new(miette::Severity::Advice, msg) }
	...
	pub fn set_help(&mut self, help: &str) { self.help_text = Some(help.into()); }

	/// Set the error code
	pub fn set_error_code(&mut self, code: &str) { self.error_code = Some(code.into()); }
}
...
pub trait ProvidesCompilerDiagnostic: Into<CompilerDiagnostic> {
	...
	fn to_diagnostic_builder(&self) -> CompilerDiagnosticBuilder { self.to_diagnostic().into() }

	/// Returns a Miette report
	fn to_miette_report(&self) -> miette::Report { miette::Report::new(self.to_diagnostic()) }
}
...
{
	fn to_diagnostic(&self) -> CompilerDiagnostic { (*self).to_diagnostic() }
}
...
where
	T: ProvidesCompilerDiagnostic,
{
	fn from(err: T) -> Self { err.to_diagnostic() }
}
