...
impl CompiledInfo{
    pub fn new(file_name:String)->miette::Result<CompiledInfo>{
        use std::fs;
        let file = fs::read_to_string(&file_name).map_err(|_|CompilerError::FileNotFound(file_name).to_diagnostic())?;
        let compiled: CompiledInfo = serde_json::from_str(&file).map_err(|e|CompilerError::JsonError(e).to_diagnostic())?;
        Ok(compiled)
    }
