...
use super::{VariableDeclared, CombinedQualifiers, SemanticError};
#[derive(Debug)]
...
impl ModuleDeclarationScope {
	...
	pub fn is_generic(&self) -> bool {
		for (var, _loc) in self.variables.values() {
			match var.specifier {
				crate::parser::ast::TypeSpecifier::Int { .. } | crate::parser::ast::TypeSpecifier::Bool { .. } => return true,
				_ => continue,
			};
		}
		false
						...
						match exprs.len(){
							...
							_ => return Err(miette::Report::new(
								SemanticError::ForbiddenExpressionInSyncOrComb
									.to_diagnostic_builder()
									.label(*span, format!("This variable's {:?} sync list contains more than two variables",id_table.get_by_key(name).unwrap()).as_str())
									.label(*location, "\"sync\" qualifier must have at most consists of one variable and its negation")
									.build(),
							))
						}
					},
					CombinedQualifiers {comb: Some((expressions,span1)), ..} => {
						for expr in expressions{
							let name2 = expr.get_name_sync_or_comb().map_err(|e| e)?;
							if name2 == *name{
								return Err(miette::Report::new(SemanticError::VariableReferencingItself
									.to_diagnostic_builder()
									.label(*span, format!("This variable's {:?} comb list contains itself",id_table.get_by_key(name).unwrap()).as_str())
