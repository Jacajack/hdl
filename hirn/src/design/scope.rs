...
impl Scope {
	...
	fn add_conditional_scope(&mut self, condition: Expression, child: ScopeId, else_scope: Option<ScopeId>) -> Result<ScopeId, DesignError> {
...
impl ScopeHandle {
	...
	pub fn if_scope(&mut self, condition: Expression) -> Result<ScopeHandle, DesignError> {
		...
		this_scope!(self).add_conditional_scope(condition, child.id(), None).unwrap();
		Ok(child)
	}
	...
	pub fn if_else_scope(&mut self, condition: Expression) -> Result<(ScopeHandle, ScopeHandle), DesignError> {
		...
		this_scope!(self).add_conditional_scope(condition, child.id(), Some(else_scope.id())).unwrap();
		Ok((child, else_scope))
	}
