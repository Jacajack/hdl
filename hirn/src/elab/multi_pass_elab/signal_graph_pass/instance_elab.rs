...
use assumptions::{ElabAssumptionsBase, ElabAssumptions};
use log::warn;

use crate::{design::{BlockInstance, Register, ModuleInstance, HasSensitivity, SignalId, EvalAssumptions}, elab::{assumptions, ElabMessageKind}};

use super::SignalGraphPassCtx;

impl SignalGraphPassCtx {
	fn elab_instance_port(&mut self, sig_id: SignalId, drive: bool, assumptions: Arc<dyn ElabAssumptionsBase>) -> Result<(), ElabMessageKind> {
		// Ignore generics
	...
	fn elab_register(&mut self, reg: &Register, assumptions: Arc<dyn ElabAssumptionsBase>) -> Result<(), ElabMessageKind> {
	...
	fn elab_module_instance(&mut self, instance: &ModuleInstance, assumptions: Arc<dyn ElabAssumptionsBase>) -> Result<(), ElabMessageKind> {
		...
		for (port_name, sig_id) in instance.get_bindings() {
			let interface_sig = module_handle.get_interface_signal_by_name(&port_name).expect("Invalid port name");
			...
			if inner_sig.is_generic() {
				inner_assumptions.assume(
					inner_sig_id, 
					assumptions
						.get(*sig_id).expect("Cannot bind unassumed signal to instance")
						.clone()
				);
			}
			else
	...
	pub(super) fn elab_block(&mut self, block: &BlockInstance, assumptions: Arc<dyn ElabAssumptionsBase>)
